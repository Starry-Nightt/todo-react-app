...
function TodoInput({
  ...
  return (
    <div className="flex w-full bg-neutral-100 rounded-lg mb-4 max-w-2xl">
      <select
      className="select font-semibold text-lg"
        value={todoPriority}
        onChange={(e) => setTodoPriority(Number(e.target.value))}
      >
        <option className="py-2" value={TodoPriority.LOW}>Low</option>
        <option className="py-2" value={TodoPriority.NORMAL}>Normal</option>
        <option className="py-2" value={TodoPriority.HIGH}>High</option>
      </select>
      <input
      ...
      />
      <button className="btn bg-white border-none" onClick={handleCreateTodo}>Add</button>
    </div>
  );
}
